    ...
    try:
        ...
        if len(signals) >= 5:
            ...
            if win_short > win_long + 10:
                holding_advice = f"⚡ 【波段高抛低吸】: 短线(1-2周)胜率 **{round(win_short,1)}%**，长线胜率降至 {round(win_long,1)}%！**见好就收，切忌死扛！**"
...
def draw_easy_kline_chart(df_raw, symbol, timeframe, support_price, resistance_price):
    """
    【极简实战易懂 K 线图】:
    1. 绿色天花板阻力带 + 红色地板防守带直接画在图上
    2. 蓝色 20 周期主力均线
    3. 屏蔽拖拽与手势冲突，保证手机顺畅滑动
    """
    try:
        if "日K" in timeframe:
            df = df_raw.tail(40).copy()
            date_fmt = '%m-%d'
        elif "周K" in timeframe:
            df = df_raw.resample('W-FRI').agg({'Open': 'first', 'High': 'max', 'Low': 'min', 'Close': 'last', 'Volume': 'sum'}).dropna().tail(35).copy()
            date_fmt = '%m/%d'
...
for idx, symbol in enumerate(st.session_state["my_portfolio"]):
    ...
    if curr_price >= ma20_now and curr_price < high_20 * 0.95:
        chart_diagnosis = "🟢 <b>形态诊断</b>: 运行于主力线上方，且距离阻力位有盈利空间，**多头良性蓄势！**"
    elif curr_price >= high_20 * 0.95:
        chart_diagnosis = "🔴 <b>形态诊断</b>: 逼近红色天花板阻力带，**抛压沉重，切勿追高！**"
    else:
        chart_diagnosis = "🟡 <b>形态诊断</b>: 跌破主力线，正在测试绿色防守底线，**密切观察企稳信号。**"
    ...
    with col:
        card_html = (
            ...
            f"</div>"
            f"<div style='font-size:11px;color:#38bdf8;background:#031428;border:1px solid #075985;padding:5px 8px;border-radius:6px;margin-bottom:4px;line-height:1.3;'>"
            f"{chart_diagnosis}"
            f"</div>"
            f"<div style='display:flex;justify-content:space-between;font-size:10px;color:#94a3b8;margin-bottom:2px;padding:0 2px;'>"
            f"<span>📈 <b>{st.session_state['global_timeframe']} 实战形态</b></span>"
            f"<span>🔵主力线 | 🔴天花板(${high_20}) | 🟢地板支撑(${low_20})</span>"
            f"</div>"
            f"</div>"
        )
